...
function read_all_data($WHERE, $ORDER, $LIMIT)
{
    ...
    $STMT = $_SQL->prepare("SELECT `CID`, `DID`, `IID`, `PID`, `DESCRIPTION`, `SUB`, `DIRECTION`, `ACCESS`, 
						...
						FROM all_data_view  
						WHERE STANDARD = 1  AND ".$WHERE."  ".$ORDER."  ".$LIMIT." ");
	$STMT->execute();
...
if($_COUNT == 0){ 
	...
	foreach ($_DATA as $dt) {
		...
		$row[3] = "<p class='grid-view-td' style='min-width:80px'>".(string)$dt["ACCESS"]."</p>";
		$row[4] = "<p class='grid-view-td' style='min-width:80px'>".(string)$dt["CUSTOMER"]."</p>";
		$row[5] = "<p class='grid-view-td' style='min-width:100px'>".(string)$dt["LOCATION"]."</p>";
		$row[6] = "<p class='grid-view-td' style='min-width:70px'>".(string)$dt["SUB"]."</p>";
		$row[7] = "<p class='grid-view-td' style='min-width:110px'>".(string)$dt["TYPE"]."</p>";
		$row[8] = "
		<a onclick=\"loadPopUp('interface_description', 'id=".(string)$dt["IID"]."')\" target='_blank' title='ID : ".$dt["IID"].", View Interface History'><i class='fa fa-history grid-history-icon'></i></a>
		<a onclick=\"loadPopUp('interface_description', 'id=".(string)$dt["IID"]."')\" target='_blank' title='ID : ".$dt["IID"].", View Interface History'><i class='fa fa-history grid-history-icon'></i></a>
		<a onclick=\"loadPopUp('interface_description', 'id=".(string)$dt["IID"]."')\" target='_blank' title='ID : ".$dt["IID"].", View Interface History'><i class='fa fa-history grid-history-icon'></i></a>";
		$_MATCH[$i] = $row;
		$i = $i + 1;
